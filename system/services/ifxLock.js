...
module.exports = function () {
  ...
  const createLockFunctions = function (name, number) {
    const isValid = function () {
      return validateLock(name, number)
    }

    const renew = function (expiration = 60 * 1000) {
      if (isValid()) {
        renewLock(name, number, expiration)
      }
    }

    const release = function (expiration = 60 * 1000) {
      if (isValid()) {
        releaseLock(name, number, expiration)
      }
    }
  ...
  const failLock = function (name, expiration = 60 * 1000) {
    ...
    if (isLocked(lock)) {
      ...
      lock.queue = []
      return createLockFunctions(name, lock.number)
    }
  }

  const awaitLock = function (name, expiration = 60 * 1000) {
    const lock = findLock(name)
  ...
  const renewLock = function (name, number, expiration = 60 * 1000) {
